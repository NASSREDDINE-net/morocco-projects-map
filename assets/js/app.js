...
function openModal(id){
  ...
  // V5.1.1: render first, then reveal the modal so the user never sees an empty dialog.
  modal.hidden=false;
  modal.style.display="grid";
  modal.setAttribute("aria-hidden","false");
  const card=modal.querySelector(".modal-card");
  if(card) card.setAttribute("aria-labelledby","modalTitle");
...
async function init(){
  try{
    ...
    if(!Array.isArray(payload)) throw new Error("Invalid projects.json: expected an array.");
    state.projects=payload;state.filtered=[...state.projects];
